...
namespace _42_IntactOwnEvent
{
    ...
    {
        private OrderEventHandler orderEventHandler;  // 声明委托字段  ≠  声明委托
        ...
        public void OnOrder(Dishes dish, Sizes size)  // 为了演示，改为了 public 访问权限
        {
            ...
            if (this.orderEventHandler != null)  // 如果orderEventHandler有绑定的 [委托实例]
            {
            ...
            // 事件只能出现在+=或-=操作符的左边，即只能add或remove，不能调用 [.]方法，即不能调用委托的相关方法
            //if (this.Order != null)  // The event 'Customer.Order' can only appear on the left hand side of += or -=
